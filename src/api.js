...
export async function api(url,options={}){
 ...
 const r=await fetch(url,{...options,headers:{'Content-Type':'application/json','X-CSRF-Token':csrf,...options.headers},body:options.body?JSON.stringify(options.body):undefined});const result=await r.json();if(!r.ok){if(r.status===401&&url!=='/api/login')dispatchEvent(new Event('navigator-session-expired'));const error=Error(result.error||'Ошибка сервера');error.status=r.status;throw error;}if(result.csrf)csrf=result.csrf;return result;
}
