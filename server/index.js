...
import {registerLegalQuestions} from './legal-questions.js';
...
function validate(m){
 if(m.expectedResult!==undefined&&(typeof m.expectedResult!=='string'||m.expectedResult.length>5000))throw Error('Результат: до 5000 символов');
 if(m.changeLinks!==undefined){if(!Array.isArray(m.changeLinks)||m.changeLinks.length>100)throw Error('Некорректные связи');const feed=JSON.parse(fs.readFileSync(path.join(privateDir,'changes.json'),'utf8'));for(const link of m.changeLinks)if(!feed.items.some(c=>c.id===link.changeId)||!['review','updated'].includes(link.status)||typeof link.action!=='string'||!link.action.trim()||link.action.length>5000||!/^https:\/\//.test(link.sourceUrl||''))throw Error('Для связи нужны публикация, описание действия и HTTPS-ссылка на проверенный источник');}
 ...
 if(m.status==='published'&&m.kind==='card'&&(!m.steps.length||m.steps.some(s=>typeof s.text!=='string'||!s.text.trim())))throw Error('Опубликованной карточке нужны заполненные шаги');
 ...
 for(const s of m.steps){if(s.documentGuidance!==undefined){if(!s.documentGuidance||typeof s.documentGuidance!=='object'||Array.isArray(s.documentGuidance))throw Error('Некорректные пояснения к документам');for(const [id,g] of Object.entries(s.documentGuidance)){if(!g||typeof g!=='object'||['when','who','required'].some(k=>g[k]!==undefined&&(typeof g[k]!=='string'||g[k].length>5000)))throw Error('Пояснения к файлу: текст до 5000 символов');}}if(typeof s.id!=='string'||typeof s.text!=='string'||!Array.isArray(s.attachments||[])||!Array.isArray(s.branches||[]))throw Error('Некорректный шаг');for(const b of s.branches||[])if(!Number.isInteger(b.index)||b.index<0||b.index>=m.steps.length)throw Error('Развилка ссылается на отсутствующий шаг');}
...
app.put('/api/admin/users/:id',admin,(req,res)=>{if(!['reader','editor','admin'].includes(req.body.role))return error(res,400,'Неверная роль');if(req.params.id===req.user.id)return error(res,400,'Нельзя изменить собственную роль');if(!one('SELECT id FROM users WHERE id=?',req.params.id))return error(res,404,'Пользователь не найден');run('UPDATE users SET role=? WHERE id=?',req.body.role,req.params.id);res.json({ok:true});});
...
registerDocuments(app,{editor});
registerLegalQuestions(app,{auth,editor});
