...
for(const name of fs.readdirSync(path.join(root,'migrations')).filter(n=>n.endsWith('.sql')).sort()){
 // Старые схемы создаются повторно; добавление столбца выполняется один раз.
 if(name==='004.sql'&&db.prepare('PRAGMA table_info(legal_questions)').all().some(c=>c.name==='context'))continue;
