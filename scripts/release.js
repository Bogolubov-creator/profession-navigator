...
const out=path.join(root,'public/release');fs.mkdirSync(out,{recursive:true});fs.rmSync(path.join(out,'files'),{recursive:true,force:true});
...
const files={};for(const m of items.filter(m=>m.fileId&&m.accessLevel!=='subscriber')){
...
fs.writeFileSync(path.join(out,'catalog.json'),JSON.stringify(sanitizeRelease({generated:new Date().toISOString(),mode:'read-only',professions,materials:items,relations,files})));
